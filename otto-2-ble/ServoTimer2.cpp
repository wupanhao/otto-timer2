...
{	unsigned int pulsewidth;
	pulsewidth = (value/9.0)*100+500;
	if(pulsewidth <= MAX_PULSE_WIDTH && pulsewidth >= MIN_PULSE_WIDTH)
...
void ServoTimer2::SetPosition(int position){
	unsigned int pulsewidth;
	pulsewidth = (position-500)/9.0*100;
	if(pulsewidth <= MAX_PULSE_WIDTH && pulsewidth >= MIN_PULSE_WIDTH)
	write(pulsewidth);
} 
